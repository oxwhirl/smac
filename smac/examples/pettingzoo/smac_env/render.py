...
class Renderer:
    def __init__(self, env, map_size, mode):
        ...
        self.screen_tranform = transform.Chain(self._world_to_world_tl, self._world_tl_to_screen)
        ...
        if mode == "human":
            ...
            add_surface(None, point.Rect(point.origin, window_size_px), self.screen_tranform, None, self.draw_screen)        
    ...
    def render(self, mode, reward, frames):
        ...
        self._game_times.append(
            (now - self._last_time,
            max(1, self.obs.observation.game_loop - self.obs.observation.game_loop)))
            ...
            for o in u.orders:
                ...
                if o.HasField("target_unit_tag"):
                    if unit_dict is None:
                        unit_dict = {t.tag: t for t in self.obs.observation.raw_data.units}
    ...
    def draw_overlay(self, surf):
        ...
        surf.write_screen(
            ...
            "Score: %s, Step: %s, %.1f/s, Time: %d:%02d" % (
                self.score, self.step, sum(steps) / (sum(times) or 1),
                sec // 60, sec % 60),
            align="right")
        surf.write_screen(
