...
class smac_parallel_env(ParallelEnv):
	def __init__(self, env, max_cycles):
		...
		self.env = env
		env.reset()
		self.agents, self.action_spaces = self._init_agents(env)
	...
	def _init_agents(self, env):
		last_type = ''
		agents = []
		action_spaces = {}
		self.agents_id = {}
		for agent_id, agent_info in env.agents.items():
			unit_action_space = spaces.Discrete(env.get_total_actions()-1) #no-op in dead units is not an action
			if agent_info.unit_type == env.marine_id:
				...
				agent_type = 'medivac'
				unit_action_space = spaces.Discrete((env.n_agents-1)+(env.n_actions_no_attack-1)) #action heal each ally unit and no-op in dead units is not an action
	...
	def step(self, all_actions):
		...
		for agent in self.possible_agents:
			...
			if agent in all_actions:
				if all_actions[agent] == None:
					action_list[agent_id] = 0
				else:
					action_list[agent_id] = all_actions[agent]+1
		reward, terminated, smac_info = self.env.step(action_list)
		self.frames += 1
		...
		all_rewards = self._all_rewards(reward)
